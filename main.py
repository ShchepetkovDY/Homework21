...
def storage_status():
    """
    Функция вывода текущего состояния по складам и получение строки команды
    """
    print(f"В склад хранится:")
    [print(v, k) for k, v in store.get_items().items()]
    print(f"\nВ магазин хранится:")
    [print(v, k) for k, v in shop.get_items().items()]
    print("Необходимо ввести команду типа - Доставить 3 печеньки из склад в магазин")
    command = input("\nВведите команду: ")
...
def main(object_from, object_to, verification=None):
    ...
    Args: object_from - объект "откуда" доставляется товар
    object_to - объект "куда" доставляется товар
    verification - дополнительный параметр, который нужен для проверки магазина на наличие товара при полном его
    заполнении
    """
    try:
